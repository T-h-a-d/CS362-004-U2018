...
int main(int argc, char **argv)
{
    ...
    int cardsDrawn = 4, cardsDiscarded = 1, cardsShuffled = 0, coinsGained = 0;

    initializeGame(2, cards, 1000, &state);

    //Test if current player
    printf("Player Draws 4 Cards and Gains a Buy...\n");
    ...
    //Test that player who played council room was awarded an additional buy
    ...
    //Test that other player was allowed to draw a card
    printf("Checking for State Change in Other Players...\n");

    printf("Hand count = %d, Expected = %d\n", testState.handCount[player + 1], state.handCount[player + 1] + 1);
    if (testState.handCount[player + 1] == state.handCount[player + 1] + 1)
        printf("** Test Passed **\n\n");
    else
        printf("** Test Failed **\n\n");

    printf("Deck count = %d, Expected = %d\n", testState.deckCount[player + 1], state.deckCount[player + 1] - 1);
    if (testState.deckCount[player + 1] == state.deckCount[player + 1] - 1)
        printf("** Test Passed **\n\n");
    else
        printf("** Test Failed **\n\n");

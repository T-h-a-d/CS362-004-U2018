...
int main(int argc, char **argv)
{
    ...
    int cardsDrawn = 2, cardsDiscarded = 1, cardsShuffled = 0, coinsGained = 0;

    int handCountFailed = 0, deckCountFailed = 0, coinCountFailed = 0;
    ...
    for(int i = 0; i < 200000; i++){
        ...
        printf("Player Recieves 2 Treasure Cards in Hand...\n");

        printf("Hand count = %d, Expected = %d\n", testState.handCount[playerTurn], state.handCount[playerTurn] + cardsDrawn - cardsDiscarded);
        if (testState.handCount[playerTurn] == state.handCount[playerTurn] + cardsDrawn - cardsDiscarded)
            printf("** Test Passed **\n\n");
        else
        {
            printf("** Test Failed **\n\n");
        ...
        printf("Deck count = %d, Expected = %d\n", testState.deckCount[playerTurn], state.deckCount[playerTurn] - cardsDrawn + cardsShuffled);
        if (testState.deckCount[playerTurn] == state.deckCount[playerTurn] - cardsDrawn + cardsShuffled)
            printf("** Test Passed **\n\n");
        else
        {
            printf("** Test Failed **\n\n");
    ...
    printf("Adventurer Failed Hand Count: %d times\n", handCountFailed);
    printf("Adventurer Failed Deck Count: %d times\n", deckCountFailed);
    printf("Adventurer Failed Coin Count: %d times\n", coinCountFailed);
}